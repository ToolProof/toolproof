...
// Wrap the retriever to add logging
const originalGetRelevantDocuments = retriever.getRelevantDocuments.bind(retriever);

retriever.getRelevantDocuments = async (query) => {
  console.log("Retriever received query:", query); // Log the query
  const results = await originalGetRelevantDocuments(query);
  console.log("Retriever returned documents:", results); // Optionally log the results
  return results;
};

...
export const resourceDescriptions: Record<ResourceNameType, string> = {
    Agent: 'Built around a powerful, reflective GPT-style LLM, such as OpenAI\'s o3, the Agent is pre-configured to use its parametric capabilities to collaborate with humans and tools for drug discovery focused on a specified disease. By leveraging OpenAI\'s structured-outputs feature, the Agent can directly generate InnerInput in the formats required by respective Tools tools.',
    ...
    InnerInput: 'InnerInput are drugs that the Agent suggests. InnerInput are represented as .pdb (Protein Data Bank) files or SMILES strings, both used to depict molecular structures.',
    ...
    OuterOutput: 'Actionable academic OuterOutput that document the Agent\'s reasoning behind crafting the Candidate, the Tools process, and the InnerOutput, offering suggestions for further action or future research (e.g. lab experiments).',
...
export const pathDescriptions: Record<number, string> = {
    ...
    3: 'The Tools tool retrieves the Candidate and generates InnerOutput.',
    4: 'The Agent retrieves the InnerOutput. If they are promising, it decides to draft a Paper.',
    5: 'The Human retrieves the Paper to read it.',
    6: 'The Human and the Agent discuss the Paper.',
    7: 'In addition: the Agent can retrieve OuterOutput and generate OuterInput for new iterations.',
};
